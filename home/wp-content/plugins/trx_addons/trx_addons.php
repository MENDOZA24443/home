...
// Current version
if ( ! defined( 'TRX_ADDONS_VERSION' ) ) define( 'TRX_ADDONS_VERSION', '2.2.1' );
...
if ( !function_exists( 'trx_addons_load_scripts_external' ) ) {
	...
	function trx_addons_load_scripts_external() {
		...
		// TweenMax library
		if ( apply_filters( 'trx_addons_filter_load_tweenmax', false ) ) {
...
if ( ! function_exists( 'trx_addons_load_frontend_scripts' ) ) {
	add_action( 'trx_addons_action_load_scripts_front', 'trx_addons_load_frontend_scripts', 1, 2 );
	function trx_addons_load_frontend_scripts( $force = false, $slug = '' ) {
		global $TRX_ADDONS_STORAGE;
		$TRX_ADDONS_STORAGE['enqueue_list'][$slug] = 1;
...
if ( ! function_exists( 'trx_addons_need_frontend_scripts' ) ) {
	function trx_addons_need_frontend_scripts( $slug ) {
		global $TRX_ADDONS_STORAGE;
		return isset( $TRX_ADDONS_STORAGE['enqueue_list'][$slug] );
	}
}
...
if ( !function_exists( 'trx_addons_load_scripts_responsive' ) ) {
	...
	function trx_addons_load_scripts_responsive() {
		// If 'debug_mode' is off - load merged styles and scripts
		if ( trx_addons_is_off(trx_addons_get_option('debug_mode')) ) {
			wp_enqueue_style( 'trx_addons-responsive', trx_addons_get_file_url('css/__responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'main', 'xl' ) );
		} else {
			...
			if ( ! empty( $TRX_ADDONS_STORAGE['capture_head_html'] ) ) {
				...
				if ( ! empty( $TRX_ADDONS_STORAGE['capture_links'] ) ) {
					$TRX_ADDONS_STORAGE['capture_head_html'] = preg_replace_callback(
						// Insert styles after the tag below
						'/<link [^>]*(href=[\'"][^\'"]*' . $theme_reg . '[^\'"]*'
										. '(__custom.css|__custom-inline.css)'
										. '[^\'"]*[\'"])[^>]*>/',
						function( $matches ) {
							if ( ! empty( $matches[0] ) ) {
								global $TRX_ADDONS_STORAGE;
