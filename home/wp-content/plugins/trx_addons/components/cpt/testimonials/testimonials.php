...
if ( !function_exists( 'trx_addons_cpt_testimonials_load_scripts_front' ) ) {
	...
	function trx_addons_cpt_testimonials_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && (
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'cpt_testimonials' );
		}
	}
}
...
if ( !function_exists( 'trx_addons_cpt_testimonials_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cpt_testimonials_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_cpt_testimonials_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_CPT . 'testimonials/testimonials.css' );
		}
		return $styles;
	}
}
...
//	add_filter( 'trx_addons_filter_get_menu_cache_html', 'trx_addons_cpt_testimonials_check_in_html_output', 10, 1 );
	...
	function trx_addons_cpt_testimonials_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'cpt_testimonials' )
			&& ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) )
		) {
