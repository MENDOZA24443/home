...
// Add users role and capabilities for "Properties" and "Properties Agents"
if (!function_exists('trx_addons_cpt_properties_add_roles_and_caps')) {
	...
	function trx_addons_cpt_properties_add_roles_and_caps() {
		...
		add_role(
			...
			esc_html__( 'Properties Agent','booked' ),
			array(
				'read'         => true,
				'edit_posts'   => false,
				'upload_files' => true,
		...
		// Add caps to roles
		trx_addons_add_capabilities( array( 'administrator', 'editor' ), array( 'property', 'properties_agent' ) );
		trx_addons_add_capabilities( array( 'author', 'trx_addons_properties_agent' ), 'property', array( '_others' ) );
...
if ( ! function_exists( 'trx_addons_cpt_properties_allow_admin_access' ) ) {
	add_filter( 'trx_addons_filter_allow_admin_access', 'trx_addons_cpt_properties_allow_admin_access', 10, 2 );
	function trx_addons_cpt_properties_allow_admin_access( $allow, $roles ) {
...
if ( !function_exists( 'trx_addons_cpt_properties_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cpt_properties_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_cpt_properties_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_CPT . 'properties/properties.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_cpt_properties_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cpt_properties_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_cpt_properties_add_editor_style_responsive( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_CPT . 'properties/properties.responsive.css' );
		}
		return $styles;
	}
}
