...
<div data-post-id="<?php the_ID(); ?>" class="sc_portfolio_item sc_item_container post_container<?php
	...
	if ( $use_gallery ) {
		if ( ! empty( $meta['video'] ) ) {
			?>
			data-video="<?php
				echo esc_attr( trx_addons_get_video_layout( array(
																'link' => $meta['video'],
																'autoplay' => true,
																'mute' => false,
																'show_cover' => false
		...
		data-details="<?php
			echo esc_attr( '<div class="post_details">'
								...
								. '<div class="post_description">'
									...
									. '<a href="' . esc_url( $link ) . '" class="theme_button post_readmore"><span class="post_readmore_label">' . esc_html__( 'Learn more', 'trx_addons' ) . '</span></a>'
								. '</div>'
							. '</div>'
						);
		?>"
