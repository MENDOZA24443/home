...
if ( !function_exists( 'trx_addons_cpt_services_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cpt_services_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_cpt_services_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_CPT . 'services/services.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_cpt_services_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cpt_services_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_cpt_services_add_editor_style_responsive( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_CPT . 'services/services.responsive.css' );
		}
		return $styles;
	}
}
		...
		if ( defined('TRX_ADDONS_CPT_SERVICES_PT') ) {
			if (is_single() && get_post_type()==TRX_ADDONS_CPT_SERVICES_PT) {
				...
				if (!empty($meta['product']) && (int) $meta['product'] > 0) {
					...
					$title['link_text'] = ! empty( $meta['price'] ) ? sprintf( __('Buy for %s', 'trx_addons'), $meta['price'] ) : __('Order now', 'trx_addons');
				}
/*
