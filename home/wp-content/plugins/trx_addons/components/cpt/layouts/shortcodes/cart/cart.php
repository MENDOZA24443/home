...
if ( !function_exists( 'trx_addons_cpt_layouts_cart_load_scripts_front' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_cpt_layouts_cart_load_scripts_front', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY);
...
if ( !function_exists( 'trx_addons_cpt_layouts_cart_load_responsive_styles' ) ) {
	...
	function trx_addons_cpt_layouts_cart_load_responsive_styles() {
		if (trx_addons_exists_page_builder() && trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-sc_layouts-cart-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'cart/cart.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'cpt-layouts-cart', 'md' ) );
		}
	}
}
