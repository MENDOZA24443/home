...
if ( !function_exists( 'trx_addons_widget_aboutme_load_scripts_front' ) ) {
	...
	function trx_addons_widget_aboutme_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && (
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'widget_aboutme' );
		}
	}
}
...
if ( !function_exists( 'trx_addons_widget_aboutme_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_widget_aboutme_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_widget_aboutme_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_WIDGETS . 'aboutme/aboutme.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_widget_aboutme_check_in_html_output' ) ) {
	...
	function trx_addons_widget_aboutme_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'widget_aboutme' )
			&& ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) )
		) {
