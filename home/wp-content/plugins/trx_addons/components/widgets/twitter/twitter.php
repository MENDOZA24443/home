...
class trx_addons_widget_twitter extends TRX_Addons_Widget {
	...
	function update( $new_instance, $instance ) {
		...
		$instance['embed_header'] = isset( $new_instance['embed_header'] ) ? 1 : 0;
		$instance['embed_footer'] = isset( $new_instance['embed_footer'] ) ? 1 : 0;
		$instance['embed_borders'] = isset( $new_instance['embed_borders'] ) ? 1 : 0;
		$instance['embed_scrollbar'] = isset( $new_instance['embed_scrollbar'] ) ? 1 : 0;
		$instance['embed_transparent'] = isset( $new_instance['embed_transparent'] ) ? 1 : 0;
	...
	function form( $instance ) {
		...
		$this->show_field(array('name' => 'twitter_api',
								...
								'value' => ! empty( $instance['twitter_api'] ) ? $instance['twitter_api'] : 'token',
...
if ( !function_exists( 'trx_addons_widget_twitter_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_widget_twitter_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_widget_twitter_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_WIDGETS . 'twitter/twitter.css' );
		}
		return $styles;
	}
}
