...
if ( !function_exists( 'trx_addons_cf7_need_load_scripts' ) ) {
	function trx_addons_cf7_need_load_scripts( $force ) {
		return trx_addons_exists_cf7()
				&& (
...
if ( !function_exists( 'trx_addons_cf7_load_scripts_front' ) ) {
	...
	function trx_addons_cf7_load_scripts_front( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && trx_addons_cf7_need_load_scripts( $force ) ) {
			$loaded = true;
...
if ( !function_exists( 'trx_addons_cf7_merge_styles' ) ) {
	...
	function trx_addons_cf7_merge_styles($list) {
		if ( trx_addons_exists_cf7() && trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
...
if ( !function_exists( 'trx_addons_cf7_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_cf7_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_cf7_add_editor_style( $styles ) {
		if ( trx_addons_exists_cf7() && ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'contact-form-7/contact-form-7.css' );
		}
		return $styles;
	}
}
