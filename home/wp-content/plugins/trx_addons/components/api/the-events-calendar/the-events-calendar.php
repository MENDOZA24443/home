...
// Check if current page needs to load specific styles and scripts
if ( !function_exists( 'trx_addons_tribe_events_need_load_scripts' ) ) {
	function trx_addons_tribe_events_need_load_scripts( $force ) {
		return trx_addons_exists_tribe_events()
				&& (
...
if ( !function_exists( 'trx_addons_tribe_events_load_scripts_front' ) ) {
	...
	function trx_addons_tribe_events_load_scripts_front( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && trx_addons_tribe_events_need_load_scripts( $force ) ) {
			$loaded = true;
...
if ( !function_exists( 'trx_addons_events_merge_styles' ) ) {
	...
	function trx_addons_events_merge_styles($list) {
		if ( trx_addons_exists_tribe_events() && trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
...
if ( !function_exists( 'trx_addons_tribe_events_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_tribe_events_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_tribe_events_add_editor_style( $styles ) {
		if ( trx_addons_exists_tribe_events() && ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'the-events-calendar/the-events-calendar.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_tribe_events_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_tribe_events_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_tribe_events_add_editor_style_responsive( $styles ) {
		if ( trx_addons_exists_tribe_events() && ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'the-events-calendar/the-events-calendar.responsive.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_events_get_blog_all_posts_link' ) ) {
	...
	function trx_addons_events_get_blog_all_posts_link($link='', $args=array()) {
		if ( empty($link) && trx_addons_is_tribe_events_page() && is_singular( Tribe__Events__Main::POSTTYPE ) ) {
