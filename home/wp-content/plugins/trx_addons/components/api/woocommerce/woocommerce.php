...
if ( ! function_exists( 'trx_addons_woocommerce_need_load_scripts' ) ) {
	function trx_addons_woocommerce_need_load_scripts( $force ) {
		return trx_addons_exists_woocommerce()
				&& (
...
if ( ! function_exists( 'trx_addons_woocommerce_load_scripts_front' ) ) {
	...
	function trx_addons_woocommerce_load_scripts_front( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && trx_addons_woocommerce_need_load_scripts( $force ) ) {
			$loaded = true;
...
if ( ! function_exists( 'trx_addons_woocommerce_merge_styles' ) ) {
	...
	function trx_addons_woocommerce_merge_styles( $list ) {
		if ( trx_addons_exists_woocommerce() && trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
...
if ( ! function_exists( 'trx_addons_woocommerce_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_woocommerce_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_woocommerce_add_editor_style( $styles ) {
		if ( trx_addons_exists_woocommerce() && ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'woocommerce/woocommerce.css' );
		}
		return $styles;
	}
}
...
if ( ! function_exists( 'trx_addons_woocommerce_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_woocommerce_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_woocommerce_add_editor_style_responsive( $styles ) {
		if ( trx_addons_exists_woocommerce() && ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_API . 'woocommerce/woocommerce.responsive.css' );
		}
		return $styles;
	}
}
