...
if ( !function_exists( 'trx_addons_sc_cover_merge_styles' ) ) {
	...
	function trx_addons_sc_cover_merge_styles($list) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'cover/cover.css';
		}
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_sc_cover_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_sc_cover_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_sc_cover_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'cover/cover.css' );
		}
		return $styles;
	}
}
