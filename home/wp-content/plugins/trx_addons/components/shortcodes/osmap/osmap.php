...
if ( !function_exists( 'trx_addons_sc_osmap_load_scripts' ) ) {
	...
	function trx_addons_sc_osmap_load_scripts( $force = false ) {
		...
		if ( ! $loaded && (
			...
			do_action( 'trx_addons_action_load_scripts_front', $force, 'sc_osmap' );
...
if ( ! function_exists( 'trx_addons_sc_osmap_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_sc_osmap_load_scripts_front_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && trx_addons_need_frontend_scripts( 'sc_osmap' )
			||
			...
			if ( ! trx_addons_is_preview( 'gutenberg' ) ) {
				wp_enqueue_style( 'trx_addons-sc_osmap-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.responsive.css'), array(), null, trx_addons_media_for_load_css_responsive( 'sc-osmap', 'md' ) );
			}
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_sc_osmap_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_sc_osmap_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_sc_osmap_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_sc_osmap_add_editor_style_responsive( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.responsive.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_check_in_html_output' ) ) {
	...
	function trx_addons_sc_osmap_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'sc_osmap' )
			&& ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) )
		) {
