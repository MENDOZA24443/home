	...
	function trx_addons_sc_hotspot_load_scripts_front( $force = false ) {
		...
		if ( ! $loaded && (
			...
			if ( $force !== 'gutenberg' && ! trx_addons_is_preview( 'gutenberg' ) ) {
				wp_enqueue_style(  'trx_addons-sc_hotspot', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.css' ), array(), null );
			}
			if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) )
				...
				wp_enqueue_script( 'trx_addons-sc_hotspot', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.js' ), array( 'jquery' ), null, true );
			}
			do_action( 'trx_addons_action_load_scripts_front', $force, 'sc_hotspot' );
		}
	}
}
...
if ( ! function_exists( 'trx_addons_sc_hotspot_load_scripts_front_responsive' ) ) {
	...
	function trx_addons_sc_hotspot_load_scripts_front_responsive( $force = false ) {
		static $loaded = false;
		if ( ! $loaded && (
			current_action() == 'wp_enqueue_scripts' && trx_addons_need_frontend_scripts( 'sc_hotspot' )
			||
			...
			if ( ! trx_addons_is_preview( 'gutenberg' ) ) {
				wp_enqueue_style(  'trx_addons-sc_hotspot-responsive', trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.responsive.css' ), array(), null, trx_addons_media_for_load_css_responsive( 'sc-hotspot', 'lg' ) );
			}
		}
	}
}
...
if ( ! function_exists( 'trx_addons_sc_hotspot_merge_styles' ) ) {
	...
	function trx_addons_sc_hotspot_merge_styles( $list ) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.css';
		}
		return $list;
	}
}
...
if ( ! function_exists( 'trx_addons_sc_hotspot_merge_styles_responsive' ) ) {
	...
	function trx_addons_sc_hotspot_merge_styles_responsive( $list ) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.responsive.css';
		}
		return $list;
	}
}
...
if ( ! function_exists( 'trx_addons_sc_hotspot_merge_scripts' ) ) {
	...
	function trx_addons_sc_hotspot_merge_scripts( $list ) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.js';
		}
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_sc_hotspot_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_sc_hotspot_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_sc_hotspot_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_sc_hotspot_add_editor_style_responsive' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_sc_hotspot_add_editor_style_responsive', TRX_ADDONS_ENQUEUE_RESPONSIVE_PRIORITY );
	function trx_addons_sc_hotspot_add_editor_style_responsive( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_SHORTCODES . 'hotspot/hotspot.responsive.css' );
		}
		return $styles;
	}
}
...
//	add_filter( 'trx_addons_filter_get_menu_cache_html', 'trx_addons_sc_hotspot_check_in_html_output', 10, 1 );
	...
	function trx_addons_sc_hotspot_check_in_html_output( $content = '' ) {
		if ( ! trx_addons_need_frontend_scripts( 'sc_hotspot' )
			&& ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) )
		) {
