...
if ( ! function_exists( 'trx_addons_audio_effects_merge_styles' ) ) {
	...
	function trx_addons_audio_effects_merge_styles($list) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_ADDONS . 'audio-effects/audio-effects.css';
		}
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_audio_effects_merge_scripts' ) ) {
	...
	function trx_addons_audio_effects_merge_scripts($list) {
		if ( trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$list[] = TRX_ADDONS_PLUGIN_ADDONS . 'audio-effects/audio-effects.js';
		}
		return $list;
	}
}
...
if ( ! function_exists( 'trx_addons_audio_effects_add_editor_style' ) ) {
	add_filter( 'trx_addons_filter_add_editor_style', 'trx_addons_audio_effects_add_editor_style', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY );
	function trx_addons_audio_effects_add_editor_style( $styles ) {
		if ( ! trx_addons_is_off( trx_addons_get_option( 'optimize_css_and_js_loading' ) ) ) {
			$styles[] = trx_addons_get_file_url( TRX_ADDONS_PLUGIN_ADDONS . 'audio-effects/audio-effects.css' );
		}
		return $styles;
	}
}
...
if ( !function_exists( 'trx_addons_elm_add_audio_effects_data' ) ) {
	...
	function trx_addons_elm_add_audio_effects_data($element) {
		if ( is_object($element) ) {
			...
			if ( ! empty($settings['audio_effects_allowed'])
				&& ! trx_addons_is_off($settings['audio_effects_allowed'])
				&& ( ! empty($settings['audio_effects'][0]['link']) || ! empty($settings['audio_effects'][0]['local']['url']) )
			) {
				$element->add_render_attribute( '_wrapper', array(
					'data-trx-addons-audio-effects' => json_encode( $settings['audio_effects'], JSON_UNESCAPED_SLASHES | JSON_FORCE_OBJECT ),
				) );
				trx_addons_audio_effects_load_scripts_front( true );
			}
		}
