...
if ( ! function_exists('trx_addons_secondary_image_before_show_options')) {
	...
	function trx_addons_secondary_image_before_show_options($options, $pt='') {
		if ( isset($options['secondary_image_post_types']) ) {
			$options['secondary_image_post_types']['options'] = trx_addons_get_list_secondary_image_posts_types();
		}
		return $options;
	}
}
...
if ( !function_exists( 'trx_addons_get_list_secondary_image_posts_types' ) ) {
	function trx_addons_get_list_secondary_image_posts_types($prepend_inherit=false) {
		static $list = false;
		if ($list === false) {
			$list = array();
...
if ( ! function_exists( 'trx_addons_secondary_image_merge_styles' ) ) {
	...
	function trx_addons_secondary_image_merge_styles($list) {
		$list[] = TRX_ADDONS_PLUGIN_ADDONS . "secondary-image/secondary-image.css";
		return $list;
	}
}
...
if ( ! function_exists( 'trx_addons_secondary_image_get_layout' ) ) {
	function trx_addons_secondary_image_get_layout() {
		...
		$image_url = trx_addons_get_secondary_image_url(
							apply_filters('trx_addons_filter_thumb_size', is_single() ? 'full' : trx_addons_get_thumb_size( 'masonry-big' ), 'secondary-image')
							);
		if ( ! empty( $image_url ) ) {
