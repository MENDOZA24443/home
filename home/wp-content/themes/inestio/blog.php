...
if ( function_exists( 'inestio_elementor_is_preview' ) && inestio_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			inestio_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	inestio_new_main_query( array(
		'post_type'      => inestio_get_theme_option( 'post_type' ),
		'category'       => inestio_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'inestio_filter_get_template_part', inestio_blog_archive_get_template() ) );
}
