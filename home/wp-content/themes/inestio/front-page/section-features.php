<div class="front_page_section front_page_section_features<?php
	$inestio_scheme = inestio_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $inestio_scheme ) && ! inestio_is_inherit( $inestio_scheme ) ) {
		echo ' scheme_' . esc_attr( $inestio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( inestio_get_theme_option( 'front_page_features_paddings' ) );
	if ( inestio_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$inestio_css      = '';
		$inestio_bg_image = inestio_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$inestio_anchor_icon = inestio_get_theme_option( 'front_page_features_anchor_icon' );
	$inestio_anchor_text = inestio_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( inestio_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$inestio_css      = '';
			$inestio_bg_mask  = inestio_get_theme_option( 'front_page_features_bg_mask' );
			$inestio_bg_color_type = inestio_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $inestio_bg_color_type ) {
				$inestio_bg_color = inestio_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$inestio_caption = inestio_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $inestio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $inestio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $inestio_caption, 'inestio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$inestio_description = inestio_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $inestio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $inestio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $inestio_description ), 'inestio_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! inestio_exists_trx_addons() ) {
						...
						inestio_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
