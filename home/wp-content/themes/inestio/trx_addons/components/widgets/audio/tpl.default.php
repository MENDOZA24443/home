		...
		foreach ( $media as $item ) {
			...
			?>
				...
				>
					...
					if ( ! empty( $item['cover'] ) ) {
						?>
						<div class='trx_addons_audio_image' <?php echo ' style="background-image:url(' . esc_url( $item['cover'] ) . ');"'; ?>></div>
						<?php
					}
					?>
					...
					if ( ! empty( $item['author'] ) || ! empty( $item['caption'] ) ) {
						...
						<div class="audio_info">
							...
							if ( '#' !== $now_text && count( $media ) > 1 ) {
								echo '<h5 class="audio_now_playing">' . ( ! empty( $now_text ) ? esc_html( $now_text ) : esc_html__( 'Now Playing', 'inestio' ) ) . ': </h5>';
							}
