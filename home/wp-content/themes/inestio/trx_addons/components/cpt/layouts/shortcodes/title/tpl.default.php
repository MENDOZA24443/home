			...
			if ( $need_content )  {
				...
				?><div class="sc_layouts_title_content"><?php
					...
					if (!empty($args['meta']) && is_single() )  {
						?><div class="sc_layouts_title_meta post_meta_category"><?php
							trx_addons_sc_show_post_meta('sc_layouts_title_cat', apply_filters('trx_addons_filter_post_meta_args', array(
					...
					if (!empty($args['title']) )  {
						?><div class="sc_layouts_title_title"><?php
							...
							?>
							<h2<?php trx_addons_seo_snippets('headline'); ?> class="sc_layouts_title_caption<?php echo esc_attr($trx_addons_blog_title_class); ?>"><?php
								...
								if (!empty($trx_addons_top_icon)) {
									...
									?><img src="<?php echo esc_url($trx_addons_top_icon); ?>" alt="<?php esc_attr_e('Icon', 'inestio'); ?>" <?php if (!empty($trx_addons_attr[3])) trx_addons_show_layout($trx_addons_attr[3]);?>><?php
								}
					...
					if (!empty($args['meta']) && is_single() )  {
						?><div class="sc_layouts_title_meta"><?php
							trx_addons_sc_show_post_meta('sc_layouts_title_other', apply_filters('trx_addons_filter_post_meta_args', array(
										'components' => 'categories,date,views,comments,likes',
										'seo' => true
										), 'sc_layouts', !empty($args['columns']) ? $args['columns'] : 1)
									);
