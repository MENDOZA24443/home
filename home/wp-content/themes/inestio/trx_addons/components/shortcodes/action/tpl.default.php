...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['actions'] as $item) {
		...
		?><div class="sc_action_item sc_action_item_<?php
			...
			if (!empty($args['min_height']) && $args['min_height'] > 0 && (int) $args['full_height'] == 0) echo ' sc_action_fixed_height '.trx_addons_add_inline_css_class(trx_addons_get_css_dimensions_from_values(array('min-height' => $args['min_height'])) . ';');
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e("Action's image", 'inestio'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
