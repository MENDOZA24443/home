...
jQuery( document ).ready(
	function() {
		...
		inestio_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="inestio_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.inestio_post_editor' );
					if ( link.length > 0 ) {
						inestio_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.inestio_post_editor', function(e) {
					inestio_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('inestio_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function inestio_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
