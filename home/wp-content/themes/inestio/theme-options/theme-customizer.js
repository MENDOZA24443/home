/* global inestio_color_schemes, inestio_dependencies, Color */
...
( function( api ) {
	...
	for (var i in inestio_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'inestio-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'inestio-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'inestio-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + inestio_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( inestio_customizer_vars['msg_reset_confirm'] )) {
			...
			if (inestio_customizer_vars && inestio_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( inestio_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', inestio_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', inestio_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					inestio_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							inestio_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			inestio_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			inestio_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof inestio_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-inestio-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-inestio-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in inestio_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-inestio-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-inestio-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-inestio-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'inestio-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in inestio_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (inestio_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = inestio_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = inestio_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function inestio_customizer_get_field_value(fld) {
	...
	function inestio_customizer_check_dependencies(cont) {
		cont.find( '.customize-control' ).each(
			function() {
				...
				var fld    = null, val = '', i;
				var depend = false;
				for (fld in inestio_dependencies) {
					if (fld == id) {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = inestio_customizer_get_field_value( fld );
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = inestio_customizer_get_field_value( fld );
						var num = 0;
						for (i in inestio_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function inestio_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( inestio_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					inestio_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in inestio_theme_fonts) {
					for (rule in inestio_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							inestio_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function inestio_customizer_actions(e) {
	...
	function inestio_customizer_update_theme_vars(opt, value) {
		inestio_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function inestio_customizer_update_theme_fonts(opt, rule, value) {
		inestio_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function inestio_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= inestio_customizer_vars['max_load_fonts']; i++) {
	...
	function inestio_customizer_update_css() {
		...
		var vars = inestio_customizer_add_theme_vars( inestio_theme_vars );

		// Make styles and add its to the css
		css += inestio_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = inestio_customizer_add_theme_fonts( inestio_theme_fonts );

		// Make styles and add its to the css
		css += inestio_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in inestio_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( inestio_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in inestio_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in inestio_color_schemes[scheme].colors) {
				colors[i] = inestio_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = inestio_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-inestio-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function inestio_customizer_add_theme_colors(colors) {
		if (inestio_additional_colors) {
			var clr = '', v = '';
			for (var k in inestio_additional_colors) {
				v   = inestio_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + inestio_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + inestio_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + inestio_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function inestio_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = inestio_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']              = inestio_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = inestio_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['sidebar']           = inestio_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']               = inestio_customizer_prepare_css_value( vars['gap'] );
				rez['content']           = inestio_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['koef_narrow']       = 3 / 4;
				rez['content_narrow']    = inestio_customizer_prepare_css_value( ( vars['page'] - vars['sidebar'] - vars['gap'] ) * rez['koef_narrow'] );
				rez['padding_narrow']    = inestio_customizer_prepare_css_value( ( vars['page'] - vars['sidebar'] - vars['gap'] ) * ( 1 - rez['koef_narrow'] ) );
				rez['sidebar_gap']       = inestio_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['gap_prc']           = vars['gap'] / vars['page'];
				rez['page_1_1']          = inestio_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = inestio_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = inestio_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = inestio_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = inestio_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = inestio_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins              = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = inestio_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = inestio_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function inestio_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function inestio_customizer_prepare_html_value(val) {
