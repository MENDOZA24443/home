...
if ( ! function_exists( 'inestio_need_page_title' ) ) {
	function inestio_need_page_title() {
		return ! is_front_page() && apply_filters( 'inestio_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'inestio_show_layout' ) ) {
	function inestio_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'inestio_get_logo_image' ) ) {
	function inestio_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = inestio_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = inestio_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = inestio_is_on( inestio_get_theme_option( 'logo_retina_enabled' ) )
						? inestio_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? inestio_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? inestio_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'inestio_get_header_video' ) ) {
	function inestio_get_header_video() {
		$video = '';
		if ( apply_filters( 'inestio_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( inestio_check_theme_option( 'header_video' ) ) {
				$video = inestio_get_theme_option( 'header_video' );
				if ( (int) $video > 0 ) {
...
if ( ! function_exists( 'inestio_show_post_featured_image' ) ) {
	function inestio_show_post_featured_image( $args = array() ) {
		...
		// Featured image
		if ( ! inestio_sc_layouts_showed( 'featured' ) && strpos( inestio_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'inestio_action_before_post_featured' );
			inestio_show_post_featured( $args );
			do_action( 'inestio_action_after_post_featured' );
		} elseif ( inestio_is_on( inestio_get_theme_option( 'seo_snippets' ) ) && has_post_thumbnail() ) {
			?>
			<meta itemprop="image" itemtype="<?php echo esc_attr( inestio_get_protocol( true ) ); ?>//schema.org/ImageObject" content="<?php echo esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ); ?>">
			<?php
		}
	}
}
...
if ( ! function_exists( 'inestio_show_post_title_and_meta' ) ) {
	function inestio_show_post_title_and_meta( $args = array() ) {
		...
		// Title and post meta
		if ( ( ! inestio_sc_layouts_showed( 'title' ) || ! inestio_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				$meta_components = inestio_array_delete_by_value( $meta_components, 'categories' );
				$meta_components = inestio_array_delete_by_value( $meta_components, 'edit' );
				$meta_components = inestio_array_delete_by_value( $meta_components, 'likes' );
				$share_position  = inestio_array_get_keys_by_value( inestio_get_theme_option( 'share_position' ) );
				if ( ! in_array( 'top', $share_position ) ) {
					$meta_components = inestio_array_delete_by_value( $meta_components, 'share' );
				}
				$seo = inestio_is_on( inestio_get_theme_option( 'seo_snippets' ) );
				if ( ! empty( $args['show_title'] ) ) {
					if ( $show_categories && ! inestio_sc_layouts_showed( 'postmeta' ) && inestio_is_on( inestio_get_theme_option( 'show_post_meta' ) ) ) {
					...
					// Post title
					if ( ! inestio_sc_layouts_showed( 'title' ) ) {
						the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
					}
					// Post subtitle
					$post_subtitle = inestio_get_theme_option( 'post_subtitle' );
					if ( ! empty( $post_subtitle ) ) {
						?>
						<div class="post_subtitle">
							<?php inestio_show_layout( $post_subtitle ); ?>
						</div>
						<?php
					}
				}
				// Post meta
				if ( ! empty( $args['show_meta'] ) && ! inestio_sc_layouts_showed( 'postmeta' ) && inestio_is_on( inestio_get_theme_option( 'show_post_meta' ) ) ) {
					if ( ! empty( $args['split_meta_by'] ) ) {
						?><div class="post_meta_other"><?php
							$meta_components = inestio_get_theme_option( 'meta_parts' );
							...
							$part1 = array_keys( inestio_array_slice( $meta_components, '', $args['split_meta_by'] ), 1 );
							$part2 = array_keys( inestio_array_slice( $meta_components, '+' . $args['split_meta_by'], '' ), 1 );
							if ( ! in_array( 'top', $share_position ) ) {
								$part2 = inestio_array_delete_by_value( $part2, 'share' );
							}
							inestio_show_post_meta(
								apply_filters(
									'inestio_filter_post_meta_args',
							...
							inestio_show_post_meta(
								apply_filters(
									...
									array_merge(
										...
										$args,
										array(
											'show_labels' => ! empty( $args['show_labels'] ) || ! in_array( 'share', $part2 ) || ! inestio_exists_trx_addons()
										)
									),
									'single',
									1
			...
			ob_end_clean();
			if ( strpos( $inestio_post_header, 'post_subtitle' ) !== false
...
if ( ! function_exists( 'inestio_show_post_content' ) ) {
	function inestio_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$simple = true;
		...
		if ( has_excerpt() ) {
			...
			inestio_show_layout( inestio_filter_post_content( get_the_content('') ) );
			...
			$quote = inestio_get_tag( inestio_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				inestio_show_layout( wpautop( $quote ) );
				$simple = false;
			} else {
				inestio_show_layout( inestio_filter_post_content( get_the_content() ) );
			}
			...
			if ( $simple ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: inestio_get_theme_option( 'excerpt_length' )
								);
				$output = inestio_excerpt( $output, $len );
			}
		}
		inestio_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'inestio_show_post_more_link' ) ) {
	function inestio_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		inestio_show_layout(
			'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read more', 'inestio' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'inestio_show_post_comments_link' ) ) {
	function inestio_show_post_comments_link( $args = array(), $otag='', $ctag='' ) {
		...
		inestio_show_layout(
			'<a class="more-link comments-link" href="' . esc_url( get_comments_link() ) . '">'
				. ( ! empty( $args['comments_text'] )
						...
						: ( $total == 0
							? esc_html__( 'Leave a comment', 'inestio' )
							: ( $total == 1 ? esc_html__( 'View comment', 'inestio' ) : esc_html__( 'View comments', 'inestio' ) )
							)
						)
			. '</a>',
			$otag,
...
if ( ! function_exists( 'inestio_show_post_pagination' ) ) {
	function inestio_show_post_pagination() {
		do_action( 'inestio_action_before_post_pagination' );
		wp_link_pages(
			array(
		...
		do_action( 'inestio_action_after_post_pagination' );
	}
}
...
if ( ! function_exists( 'inestio_show_post_footer' ) ) {
	function inestio_show_post_footer( $components = 'pages,tags,likes,share,prev_next,author' ) {
		...
		foreach( $components as $comp ) {
			...
			if ( 'tags' == $comp ) {
				...
				// Post tags
				the_tags( '<div class="post_tags_single"><span class="post_meta_label">' . esc_html__( 'Tags:', 'inestio' ) . '</span> ', '', '</div>' );
...
if ( ! function_exists( 'inestio_show_post_meta' ) ) {
	function inestio_show_post_meta( $args = array() ) {
		if ( is_single() && inestio_is_off( inestio_get_theme_option( 'show_post_meta' ) ) ) {
		...
		foreach ( $components as $comp ) {
			...
			if ( 'categories' == $comp ) {
				// Post categories
				$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'inestio_filter_get_post_categories', '' );
				if ( ! empty( $cats ) ) {
					inestio_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span>');
				}
			} elseif ( 'date' == $comp ) {
				// Post date
				$dt = apply_filters( 'inestio_filter_get_post_date', inestio_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
				if ( ! empty( $dt ) ) {
					inestio_show_layout(
						$dt,
				...
				if ( $author_id > 0 ) {
					...
					$author_avatar = ! empty( $args['author_avatar'] )
										? get_avatar( get_the_author_meta( 'user_email', $author_id ), apply_filters( 'inestio_filter_author_avatar_size', 56, 'post_meta' ) * inestio_get_retina_multiplier() ) 
										: '';
					echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
							. ( ! empty( $author_avatar )
								...
								: '<span class="post_author_by">' . esc_html__( 'By', 'inestio' ) . '</span>'
								)
				...
				if ( !is_single() || have_comments() || comments_open() ) {
					...
					echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
							. '<span class="post_meta_number">' . esc_html( inestio_num2size( $post_comments ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _n( 'Comment', 'Comments', $post_comments, 'inestio' ) ) . '</span>' : '' )
						. '</a>';
				}
				...
				if ( function_exists( 'trx_addons_get_post_views' ) ) {
					...
					echo ( is_single()
						...
						. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
							. '<span class="post_meta_number">' . esc_html( inestio_num2size( $post_views ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _n( 'View', 'Views', $post_views, 'inestio' ) ) . '</span>' : '' )
						. ( is_single()
							? '</span>'
							: '</a>'
							);
			...
			// Likes (Emotions)
				...
				if ( function_exists( 'trx_addons_get_post_likes' ) ) {
					...
					$liked = isset( $_COOKIE['trx_addons_likes'] ) ? inestio_get_value_gpc( 'trx_addons_likes' ) : '';
					...
					echo ( true == $emotions_allowed
							...
							: '<a href="#"'
								...
								. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'inestio') : esc_attr__( 'Dislike', 'inestio' ) ) . '"'
								. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
								. ' data-likes="' . esc_attr( $post_likes ) . '"'
								. ' data-title-like="' . esc_attr__( 'Like', 'inestio') . '"'
								. ' data-title-dislike="' . esc_attr__( 'Dislike', 'inestio' ) . '"'
								. '>'
							)
								. '<span class="post_meta_number">' . esc_html( inestio_num2size( $post_likes ) )  . '</span>'
								. ( $args['show_labels']
									? '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _n( 'Reaction', 'Reactions', $post_likes, 'inestio' ) )
											: esc_html( _n( 'Like', 'Likes', $post_likes, 'inestio' ) )
											)
									. '</span>'
									: '' )
							. '</a>';
				...
				// Socials share
				inestio_show_share_links(
					array(
						'type'      => $args['share_type'],
						'direction' => $args['share_direction'],
						'caption'   => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'inestio' ) : '',
				...
				// Edit page link
				edit_post_link( esc_html__( 'Edit', 'inestio' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

			} else {
				// Custom counter
				do_action( 'inestio_action_show_post_meta', $comp, get_the_ID(), $args );
			}
...
if ( ! function_exists( 'inestio_show_post_featured' ) ) {
	function inestio_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'popup'         => inestio_get_theme_option( 'video_in_popup' ), // Open video in popup
				'hover'         => inestio_get_theme_option( 'image_hover' ),    // Hover effect
				...
				'show_no_image' => inestio_is_on( inestio_get_theme_setting( 'allow_no_image' ) ),  // Display 'no-image.jpg' if post haven't thumbnail
				'seo'           => inestio_is_on( inestio_get_theme_option( 'seo_snippets' ) ),     // Add SEO-snippets
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: inestio_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? inestio_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = inestio_get_post_image();
				if ( ! empty( $image ) ) {
					$image = inestio_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . inestio_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						inestio_is_on( inestio_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
													: ''
					);
				}
				if ( inestio_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output = '<div class="post_featured' . ( $args['class'] ? ' ' . esc_attr( $args['class'] ) : '' ) . '"'
					. ( $args['seo'] ? ' itemscope="itemscope" itemprop="image" itemtype="' . esc_attr( inestio_get_protocol( true ) ) . '//schema.org/ImageObject"' : '')
				...
				if ( has_post_thumbnail() && $args['seo'] ) {
					$inestio_attr = inestio_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $inestio_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $inestio_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				|| ( ! $args['thumb_only']
						&& ( in_array( $post_format, array( 'image', 'audio', 'video' ) )
							|| ( 'gallery' == $post_format && inestio_exists_trx_addons() )
							)
					)
			) {
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
					$post_content = inestio_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format || ! empty( $args['video'] ) ) {
						$video = ! empty( $args['video'] ) ? $args['video'] : inestio_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = inestio_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = inestio_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = inestio_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							...
							if ( $has_thumb && ! empty( $args['popup'] ) && function_exists( 'trx_addons_get_video_layout' ) ) {
								...
								if ( ! empty( $popup[0] ) && ! empty( $popup[1] ) ) {
									if ( preg_match( '/<a .*<\/a>/', $popup[0], $matches ) && ! empty( $matches[0] ) ) {
										$video_out = true;
										?>
										<div class="post_video_hover post_video_hover_popup"><?php inestio_show_layout( $matches[0] ); ?></div>
										<?php
										inestio_show_layout($popup[1]);
									}
								}
							}
							if ( ! $video_out ) {
								if ( $has_thumb ) {
									$video = inestio_make_video_autoplay( $video );
									?>
						...
						$audio = inestio_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = inestio_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = inestio_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = inestio_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = inestio_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( inestio_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( inestio_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( inestio_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );
								...
								if ( ! empty( $media_author) || ! empty( $media_title) ) {
									...
									<div class="audio_info">
										...
										if ( ! empty( $media_author ) ) {
											?>
											<div class="post_audio_author"><?php inestio_show_layout( $media_author ); ?></div>
											<?php
										}
										if ( ! empty( $media_title ) ) {
											?>
											<h5 class="post_audio_title"><?php inestio_show_layout( $media_title ); ?></h5>
											<?php
										}
										?>
									</div>
								...
								// Display audio
								inestio_show_layout( $audio, '<div class="audio_wrap">', '</div>' );
								?>
							</div>
							<?php
						}
						...
						$output = inestio_get_slider_layout( $slider_args );
						if ( '' != $output ) {
							inestio_show_layout( $output );
						}
					}
...
if ( ! function_exists( 'inestio_get_no_image' ) ) {
	function inestio_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = inestio_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || inestio_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = inestio_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'inestio_add_bg_in_post_nav' ) ) {
	function inestio_add_bg_in_post_nav() {
		if ( ! is_single() || ! inestio_get_theme_setting( 'thumbs_in_navigation' ) ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), inestio_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), inestio_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		inestio_add_inline_css( $css );
	}
}

// Show related posts
if ( ! function_exists( 'inestio_show_related_posts' ) ) {
	function inestio_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : inestio_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) inestio_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = inestio_get_theme_option( 'related_position' );
			$related_style = inestio_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'You May Also Like', 'inestio' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = inestio_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = inestio_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = inestio_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'inestio_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					inestio_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						if ( ! apply_filters( 'inestio_filter_related_post_showed', false, $args, $style ) ) {
							get_template_part( apply_filters( 'inestio_filter_get_template_part', 'templates/related-posts', $style ), $style );
						}
						?></div><?php
					}
				?></div><!-- /.swiper-wrapper || .columns_wrap --><?php
				if ( $args['slider'] ) {
					inestio_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'inestio_show_related_posts_callback' ) ) {
	add_action( 'inestio_action_related_posts', 'inestio_show_related_posts_callback' );
	function inestio_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'inestio_filter_show_related_posts', false ) ) {
			$inestio_related_posts   = (int) inestio_get_theme_option( 'related_posts' );
			$inestio_related_columns = (int) inestio_get_theme_option( 'related_columns' );
			$inestio_related_style   = inestio_get_theme_option( 'related_style' );
			if ( (int) inestio_get_theme_option( 'show_related_posts' ) && $inestio_related_posts > 0 ) {
				inestio_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $inestio_related_posts ) ),
						'columns'        => max( 1, min( 6, $inestio_related_posts, $inestio_related_columns ) ),
					),
					$inestio_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'inestio_is_blog_style_use_masonry' ) ) {
	function inestio_is_blog_style_use_masonry( $style ) {
		$blog_styles = inestio_storage_get( 'blog_styles' );
		return ! empty( $blog_styles[ $style ][ 'scripts' ] ) && in_array( 'masonry', (array) $blog_styles[ $style ][ 'scripts'] );
	}
}

// Show tabs with blog filters
if ( ! function_exists( 'inestio_show_filters' ) ) {
	function inestio_show_filters( $args = array() ) {
		$args = array_merge(
			array(
				'post_type'  => '',
				'taxonomy'   => '',
		...
		if ( is_array( $terms ) && count( $terms ) > 0 ) {
			$tabs[ $args['parent_cat'] ] = esc_html__( 'All', 'inestio' );
		...
		if ( count( $tabs ) > 0 ) {
			$filters_ajax   = inestio_get_theme_setting( 'blog_filters_use_ajax' );
			$filters_active = $args['parent_cat'];
			$filters_id     = 'inestio_filters';
			?>
			<div class="inestio_tabs inestio_tabs_ajax inestio_filters">
				<ul class="inestio_tabs_titles">
					<?php
					foreach ( $tabs as $tab_id => $tab_title ) {
						?>
						<li><a href="<?php
							echo esc_url( inestio_get_hash_link( sprintf( '#%1$s_%2$s_content', $filters_id, $tab_id ) ) );
				...
				foreach ( $tabs as $tab_id => $tab_title ) {
					...
					<div id="<?php echo esc_attr( sprintf( '%1$s_%2$s_content', $filters_id, $tab_id ) ); ?>"
						class="inestio_tabs_content"
						data-blog-template="<?php echo esc_attr( inestio_storage_get( 'blog_template' ) ); ?>"
					...
					>
						<?php
						if ( $tab_need_content ) {
							inestio_show_posts( array_merge( $args, array( 'cat' => $tab_id ) ) );
						}
						?>
					</div>
					<?php
			...
			inestio_show_posts( array_merge( $args, array( 'cat' => $args['parent_cat'] ) ) );
		}
	}
}
...
if ( ! function_exists( 'inestio_show_posts' ) ) {
	function inestio_show_posts( $args = array() ) {
		$args = array_merge(
			array(
				'post_type'      => 'post',
				'taxonomy'       => 'category',
				'parent_cat'     => 0,
				'cat'            => 0,
				'posts_per_page' => (int) inestio_get_theme_option( 'posts_per_page' ),
				'page'           => 1,
				'sticky'         => false,
				'blog_style'     => '',
				'echo'           => true,
		...
		$blog_style   = empty( $args['blog_style'] ) ? inestio_get_theme_option( 'blog_style' ) : $args['blog_style'];
		...
		if ( strpos( $style, 'blog-custom-' ) === 0 ) {
			$custom_blog_id   = inestio_get_custom_blog_id( $style );
			$custom_blog_meta = inestio_get_custom_layout_meta( $custom_blog_id );
			if ( ! empty( $custom_blog_meta['margin'] ) ) {
				inestio_add_inline_css( sprintf( '.page_content_wrap{padding-top:%s}', esc_attr( inestio_prepare_css_value( $custom_blog_meta['margin'] ) ) ) );
			}
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = inestio_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
		...
		if ( $args['sticky'] ) {
			...
			if ( inestio_get_theme_option( 'first_post_large' ) && ! is_paged() && ! in_array( inestio_get_theme_option( 'body_style' ), array( 'fullwide', 'fullscreen' ) ) && have_posts() ) {
				the_post();
				get_template_part( apply_filters( 'inestio_filter_get_template_part', 'templates/content', 'excerpt' ), 'excerpt' );
			}
			?>
		...
		while ( have_posts() ) {
			...
			get_template_part( apply_filters( 'inestio_filter_get_template_part', $args['sticky'] && is_sticky() ? 'templates/content-sticky' : inestio_blog_item_get_template( $blog_style ) ) );
		}
...
// AJAX handler for the inestio_ajax_get_posts action
if ( ! function_exists( 'inestio_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_inestio_ajax_get_posts', 'inestio_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_inestio_ajax_get_posts', 'inestio_ajax_get_posts_callback' );
	function inestio_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'inestio' );
		}
...
// Show pagination
if ( ! function_exists( 'inestio_show_pagination' ) ) {
	function inestio_show_pagination( $args = array() ) {
		global $wp_query;
		$pagination = ! empty( $args[ 'pagination' ] )
						? $args[ 'pagination' ]
						: inestio_get_theme_option( 'blog_pagination' );
		$prefix     = ! empty( $args[ 'class_prefix' ] )
						? $args[ 'class_prefix' ]
						: 'nav';
		if ( 'pages' == $pagination ) {
			inestio_show_layout( str_replace( "\n", '', get_the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'inestio' ),
					'next_text'          => esc_html__( '>', 'inestio' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'inestio' ) . ' </span>',
				)
			) ) );
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="<?php echo esc_attr( $prefix ); ?>-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'inestio' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="<?php echo esc_attr( $prefix ); ?>-links-old">
				<span class="<?php echo esc_attr( $prefix ); ?>-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'inestio' ) : esc_html__( 'Newest posts', 'inestio' ) ); ?></span>
				<span class="<?php echo esc_attr( $prefix ); ?>-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'inestio' ) : esc_html__( 'Older posts', 'inestio' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'inestio_single_comments_field' ) ) {
	function inestio_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'inestio_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'inestio_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
