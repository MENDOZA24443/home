 ...
 *
 * @package INESTIO
 * @since INESTIO 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'INESTIO_THEME_FREE' ) ) {
	define( 'INESTIO_THEME_FREE', false );
}
if ( ! defined( 'INESTIO_THEME_FREE_WP' ) ) {
	define( 'INESTIO_THEME_FREE_WP', false );
}

// If this theme is a part of Envato Elements
if ( ! defined( 'INESTIO_THEME_IN_ENVATO_ELEMENTS' ) ) {
	define( 'INESTIO_THEME_IN_ENVATO_ELEMENTS', false );
}

// If this theme support external updates
if ( ! defined( 'INESTIO_THEME_ALLOW_UPDATE' ) ) {
	define( 'INESTIO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'INESTIO_ALLOW_SKINS' ) ) {
	define( 'INESTIO_ALLOW_SKINS', true );
}
if ( ! defined( 'INESTIO_DEFAULT_SKIN' ) ) {
	define( 'INESTIO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'INESTIO_REMEMBER_SKIN' ) ) {
	define( 'INESTIO_REMEMBER_SKIN', false );
}
...
$GLOBALS['INESTIO_STORAGE'] = array(
	...
	'theme_doc_url'       => '//inestio.axiomthemes.com/doc/',

	'theme_demofiles_url' => '//demofiles.axiomthemes.com/inestio/',
	...
	'theme_download_url'  => '//themeforest.net/item/inestio-business-creative-wordpress-theme/26305292',         // Axiom
...
if ( ! function_exists( 'inestio_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'inestio_importer_set_options', 9 );
	function inestio_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( inestio_get_protocol() . ':' . inestio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( inestio_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'inestio_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'inestio_ocdi_set_options', 9 );
	function inestio_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( inestio_get_protocol() . ':' . inestio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'inestio_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'inestio_customizer_theme_setup1', 1 );
	function inestio_customizer_theme_setup1() {
		...
		inestio_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'inestio_filter_allow_fullscreen' return false)
